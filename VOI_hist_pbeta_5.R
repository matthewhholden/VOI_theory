...
pdf(file = 'EVPI_hists_exp_norm_unif_p_beta_5.pdf', width = 7, height = 2.6)
ylim = c(0, max(h.u.0$counts, h.n.0$counts, h.e.0$counts))
...
plot(h.u.0, col = rgb(0,0,1,.35), ylim = ylim,
     ...
     xaxs='i', yaxs='i', 
     cex.axis = ca, lwd = lw)
text(x=.15, y = .965*ylim[2], 'a) Uniform utility', cex = cl)
plot(h.e.0, col = rgb(1,0,0,.35), ylim = ylim,
     xlab="", ylab="", main = "",
     xaxs='i', yaxs='i', yaxt = 'n', 
     cex.axis = ca, lwd = lw)
text(x=.19, y = .965*ylim[2], 'b) Exponential utility', cex = cl)
plot(h.n.0, col = rgb(0,1,0,.35), ylim = ylim,
     xlab="", ylab="", main = "",
     xaxs='i', yaxs='i', yaxt = 'n',
     cex.axis = ca, lwd = lw)
text(x=.14, y = .965*ylim[2], 'c) Normal utility', cex = cl)
...
pdf(file = 'EVPI_hists_exp_norm_unif_p_beta_5_raw.pdf', width = 7, height = 2.6)
par(mfrow = c(1,3), mar = c(1,1.2,1,1), oma= c(3,3,0,0))
lw = 1; ca=1.1; cl = 1.2;
...
plot(h.u.0.raw, col = rgb(0,0,1,.35), ylim = ylim,
     ...
     xaxs='i', yaxs='i', 
     cex.axis = ca, lwd = lw)
text(x=.66, y = .962*ylim[2], 'a) Uniform utility', cex = cl)
plot(h.e.0.raw, col = rgb(1,0,0,.35), ylim = ylim,
     xlab="", ylab="", main = "",
     xaxs='i', yaxs='i', yaxt = 'n', 
     cex.axis = ca, lwd = lw)
text(x=.81, y = .962*ylim[2], 'b) Exponential utility', cex = cl)
plot(h.n.0.raw, col = rgb(0,1,0,.35), ylim = ylim,
     xlab="", ylab="", main = "",
     xaxs='i', yaxs='i', yaxt = 'n',
     cex.axis = ca, lwd = lw)
text(x=.63, y = .962*ylim[2], 'c) Normal utility', cex = cl)
