...
plot(h.u.0, col = rgb(0,0,1,.35), ylim = ylim,
     ...
     xaxs='i', yaxs='i', 
     cex.axis = ca, lwd = lw)
text(x=.16, y = .965*ylim[2], 'a) Uniform utility', cex = cl)
plot(h.e.0, col = rgb(1,0,0,.35), ylim = ylim,
     xlab="", ylab="", main = "",
     xaxs='i', yaxs='i', yaxt = 'n', 
     cex.axis = ca, lwd = lw)
text(x=.21, y = .965*ylim[2], 'b) Exponential utility', cex = cl)
plot(h.n.0, col = rgb(0,1,0,.35), ylim = ylim,
     xlab="", ylab="", main = "",
     xaxs='i', yaxs='i', yaxt = 'n',
     cex.axis = ca, lwd = lw)
text(x=.15, y = .965*ylim[2], 'c) Normal utility', cex = cl)
...
pdf(file = 'EVPI_hists_exp_norm_unif_p_fixed_0_5_raw.pdf', width = 7, height = 2.6)
par(mfrow = c(1,3), mar = c(1,1.2,1,1), oma= c(3,3,0,0))
lw = 1; ca=1.1; cl = 1.2;
...
plot(h.u.0.raw, col = rgb(0,0,1,.35), ylim = ylim,
     ...
     xaxs='i', yaxs='i', 
     cex.axis = ca, lwd = lw)
text(x=.9, y = .965*ylim[2], 'a) Uniform utility', cex = cl)
plot(h.e.0.raw, col = rgb(1,0,0,.35), ylim = ylim,
     xlab="", ylab="", main = "",
     xaxs='i', yaxs='i', yaxt = 'n', 
     cex.axis = ca, lwd = lw)
text(x=.9, y = .965*ylim[2], 'b) Exponential utility', cex = cl)
plot(h.n.0.raw, col = rgb(0,1,0,.35), ylim = ylim,
     xlab="", ylab="", main = "",
     xaxs='i', yaxs='i', yaxt = 'n',
     cex.axis = ca, lwd = lw)
text(x=.7, y = .965*ylim[2], 'c) Normal utility', cex = cl)
...
########### pstar plot ##################
ind.VOIg0.e = which(VOI.e.prop>0)
ind.VOIg0.u = which(VOI.u.prop>0)
ind.VOIg0.n = which(VOI.n.prop>0)

hp.u = hist(pstar.u[ind.VOIg0.u])
hp.e = hist(pstar.e[ind.VOIg0.e])
hp.n = hist(pstar.n[ind.VOIg0.n])

#figure P* distributions 
pdf(file = 'EVPI_pstar_hists_exp_norm_unif.pdf', width = 7, height = 2.6)
par(mfrow = c(1,3), mar = c(1,1.2,1,1), oma= c(3.5,3,0,0))
lw = 1; ca=1.1; cl = 1.2;
ylim = c(0, 1.1*max(hp.u$counts, hp.e$counts, hp.n$counts))
plot(hp.u, col = rgb(0,0,1,.35), ylim = ylim,
     xlab="", ylab="", main = "",
     xaxs='i', yaxs='i', 
     cex.axis = ca, lwd = lw)
text(x=.3, y = .96*ylim[2], 'a) Uniform utility', cex = cl)
plot(hp.e, col = rgb(1,0,0,.35), ylim = ylim,
     xlab="", ylab="", main = "",
     xaxs='i', yaxs='i', yaxt = 'n', 
     cex.axis = ca, lwd = lw)
text(x=.33, y = .96*ylim[2], 'b) Exponential utility', cex = cl)
plot(hp.n, col = rgb(0,1,0,.35), ylim = ylim,
     xlab="", ylab="", main = "",
     xaxs='i', yaxs='i', yaxt = 'n',
     cex.axis = ca, lwd = lw)
text(x=.26, y = .96*ylim[2], 'c) Normal utility', cex = cl)
mtext(side=1, expression(paste("p"^"*")), 
      outer = TRUE, padj=1.4, cex = cl)
mtext(side=2, "Frequency", outer = TRUE, padj=-1.5, cex = cl)
dev.off()
